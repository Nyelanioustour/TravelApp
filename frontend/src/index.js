                ...
                loginSubmit.addEventListener('click', event=>{
                    ...
                    userData.forEach(user=>{
                        if (user.name === userName.value){
                            ...
                            currentUser = user
                            userFound = true
                            document.querySelector('#login-out').innerHTML = '<a class="button is-success" id="log-in-button"><strong>Log Out</strong></a>'
                            document.querySelector('#add-location-form').classList = 'box'
                            let loginButton = document.querySelector('#log-in-button')
                            loginButton.addEventListener('click',event=>{
                                location.reload()
                            })
        ...
        function renderPlace(loadingPlace){
            ...
            div.addEventListener('click',event=>{
                ...
                addPhotoButton.addEventListener('click',event=>{
                    ...
                    .then(response=>response.json())
                    .then(photo=>{
                        createAlbum(photo,loadingPlace)
                        document.querySelector('#new-photo-url').value = ""
                        document.querySelector('#add-photo-form').classList='hide'
                        let div = document.createElement('div')
                        div.classList = "carousel-item view overlay"
                        div.id = 'add-photo'
                        div.innerHTML = `<img class="d-block w-100 img-fluid" src=${img_url} alt="carousel slide" height="100" width="100">`
                        carousel.append(div)
